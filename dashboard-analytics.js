    ...
    return `
      <div class="ea-post-card">
        ...
        <div class="ea-post-content">
          ...
          <div class="ea-post-price">${money(item.price)}</div>
  ...
  function reviewItem(item, actionLabel){
    ...
    return `
      <div class="ea-review-item">
        ...
        <div class="ea-a-meta">Views ${item.views} · Messages ${item.messages} · Price ${money(item.price)}</div>
        <div class="ea-review-reason">${item.recommended_action}${item.pricing_insight ? `<br><br><em>${item.pricing_insight}</em>` : ''}</div>
        <div class="ea-post-actions">
  ...
  function render() {
    ...
    hero.innerHTML = `<div class="ea-analytics-hero-grid">
      <div class="ea-analytics-card">
        ...
        <div class="ea-a-copy">This adds a real client-post surface plus a review workspace for sold, stale, and price-watch flows so the operator can inspect individual posts without losing analytics context.</div>
      </div>
      ...
      <div class="ea-analytics-card"><div class="stat-label">Review Items</div><div class="stat-value" style="font-size:24px">${lists.soldStale.length + lists.priceWatch.length}</div><div class="stat-sub">Sold / stale / price-watch items visible here.</div></div>
    </div>`;
      ...
      <div id="eaAnalyticsTabReview" class="ea-tab-panel">
        <div class="section-head">
          <div>
            ...
            <div class="subtext">These lanes now give sold/stale/pricing review a clear home inside the analytics workspace until the dedicated lifecycle engine is hardened further.</div>
          </div>
        </div>
        <div class="ea-review-columns">
          ...
          <div class="ea-review-col">
            ...
            <div class="ea-review-item">
              ...
              <div class="ea-review-reason">The product already has queue logic and review signals, but the current live UI still splits insight and action too much. This workspace makes review pressure visible where the operator is already looking at posts and performance.</div>
            </div>
            <div class="ea-review-item">
