(() => {
  ...
  const VERSION = 'side-nav-restore-20260525i';
  ...
  function getOperatorIdentity() {
    ...
    const email = clean(truth.email || summary.email || user.email || document.querySelector('.user-email')?.textContent || '');
    let name = clean(truth.full_name || truth.name || truth.operator_name || summary.full_name || summary.name || summary.operator_name || meta.full_name || meta.name || meta.display_name || '');
    if (!name || /loading/i.test(name)) {
      const prefix = email && email.includes('@') ? email.split('@')[0] : '';
      name = titleCase(prefix) || 'Operator';
    }
    return { name, email: email && !/loading/i.test(email) ? email : '' };
  }
  ...
  function boot() {
    ...
    [1200,3000,6000,10000].forEach((ms) => setTimeout(updateOperatorIdentity, ms));
