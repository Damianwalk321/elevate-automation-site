(() => {
  ...
  const CSS = `
    ...
    .ea-post-price.pending{color:#ffcfad}
  ...
  function get(path, fallback) {
    return NS.state?.get?.(path, fallback);
  }
  ...
  function getListings() {
    ...
    return source.map((row) => {
      ...
      const priceReview = /price/i.test(lifecycle) || /price/i.test(recommended);
      return {
        ...
        pricePending: !(price > 0) && !(rawPrice > 0),
        views,
        messages,
  ...
  function postCard(item) {
    const badge = item.likely_sold ? "Likely Sold" : item.price_review ? "Price Watch" : item.stale ? "Needs Refresh" : (item.status || "Active");
    const badgeClass = item.likely_sold ? "critical" : (item.price_review ? "tracked" : (item.stale ? "estimated" : "synced"));
    return `
      <div class="ea-post-card">
        ...
        <div class="ea-post-content">
          ...
          <div class="ea-post-price ${item.pricePending ? "pending" : ""}">${item.priceLabel}</div>
          ...
          <div class="ea-review-reason">
            ...
            ${item.pricing_insight ? `<br><br><em>${item.pricing_insight}</em>` : ""}
            ${item.pricePending ? `<br><br><strong>Data note:</strong> Canonical price is unresolved. Keep this out of a hard $0 state until mapping is corrected.` : ""}
          </div>
          <div class="ea-post-actions">
            <button class="action-btn" type="button" data-ea-open="listings">Open Listing</button>
  ...
  function reviewItem(item, actionLabel) {
    const badge = item.likely_sold ? "Likely sold" : item.price_review ? "Price review" : "Stale / review";
    const badgeClass = item.likely_sold ? "critical" : (item.price_review ? "tracked" : "estimated");
    return `
      <div class="ea-review-item">
        ...
        <div class="ea-a-meta">Views ${item.views} · Messages ${item.messages} · ${item.priceLabel}</div>
        <div class="ea-review-reason">
          ${item.recommended_action}
          ${item.pricing_insight ? `<br><br><em>${item.pricing_insight}</em>` : ""}
          ${item.pricePending ? `<br><br><strong>Data note:</strong> unresolved price mapping still needs correction.` : ""}
        </div>
        <div class="ea-post-actions">
          <button class="action-btn" type="button" data-ea-open="review-center">${actionLabel}</button>
          <button class="action-btn" type="button" data-ea-source="${item.source_url}">Open Source</button>
        </div>
      </div>
    `;
  ...
  function render() {
    ...
    if (!hero) {
      ...
      hero.id = "eaAnalyticsHero";
      hero.className = "ea-analytics-hero";
      shell.appendChild(hero);
    }
    hero.innerHTML = `<div class="ea-analytics-hero-grid">
      <div class="ea-analytics-card">
        <div class="g-eyebrow">Unified Sync</div>
        <h2 class="ea-a-title">Analytics now includes both portfolio visibility and a direct client-post surface.</h2>
        <div class="ea-a-copy">Vehicle cards stay visible here, while sold, stale, and price-watch review pressure now has a cleaner operational bridge into Review Center.</div>
      </div>
      ...
      <div class="ea-analytics-card"><div class="stat-label">Client Posts</div><div class="stat-value" style="font-size:24px">${listings.length}</div><div class="stat-sub">Listings visible in analytics.</div></div>
      <div class="ea-analytics-card"><div class="stat-label">Review Items</div><div class="stat-value" style="font-size:24px">${lists.soldStale.length + lists.priceWatch.length}</div><div class="stat-sub">Sold, stale, and price-watch pressure.</div></div>
    </div>`;

    let syncCard = document.getElementById("eaAnalyticsSync");
    if (!syncCard) {
      syncCard = document.createElement("div");
      syncCard.id = "eaAnalyticsSync";
      syncCard.className = "ea-analytics-card";
      shell.appendChild(syncCard);
    }
    ...
    if (!tabs) {
      ...
      tabs.id = "eaAnalyticsTabs";
      tabs.className = "ea-analytics-card";
      ...
      <div id="eaAnalyticsTabPosts" class="ea-tab-panel">
        <div class="section-head">
          <div>
            ...
            <div class="subtext">This preserves vehicle cards inside analytics while Listings becomes the dedicated portfolio workspace.</div>
          </div>
        </div>
      ...
      <div id="eaAnalyticsTabReview" class="ea-tab-panel">
        <div class="section-head">
          <div>
            ...
            <div class="subtext">Analytics retains visibility, but the dedicated Review Center now becomes the stronger operational home for final triage.</div>
          </div>
        </div>
        <div class="ea-review-columns">
          <div class="ea-review-col">
            ...
            ${lists.soldStale.length ? lists.soldStale.map((item) => reviewItem(item, "Open Review Center")).join("") : `<div class="ea-empty">No sold or stale items right now.</div>`}
          </div>
          <div class="ea-review-col">
            <div class="ea-review-col-head"><h3>Price Watch</h3><span class="ea-pill tracked">${lists.priceWatch.length}</span></div>
            ${lists.priceWatch.length ? lists.priceWatch.map((item) => reviewItem(item, "Open Review Center")).join("") : `<div class="ea-empty">No price-watch items right now.</div>`}
          </div>
          <div class="ea-review-col">
            <div class="ea-review-col-head"><h3>Operator Notes</h3><span class="ea-pill estimated">Workspace</span></div>
            <div class="ea-review-item">
              <strong>Why this matters</strong>
              <div class="ea-review-reason">The product already has queue logic and review signals, but the live UI was still splitting insight and action too much. Bundle D keeps the analytics context while moving day-to-day review pressure into a dedicated Review Center.</div>
            </div>
            <div class="ea-review-item">
              <strong>Known blocker</strong>
              <div class="ea-review-reason">Canonical price resolution still needs deeper backend hardening. Until that is finished, unresolved rows should read as price pending, not $0.</div>
            </div>
          </div>
        </div>
      </div>
  ...
  NS.analytics = { renderBundleDAnalytics: render };
