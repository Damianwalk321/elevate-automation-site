(() => {
  if (window.__ELEVATE_DASHBOARD_PHASE11_LOADER__) return;
  window.__ELEVATE_DASHBOARD_PHASE11_LOADER__ = true;

  const NS = (window.ElevateDashboard = window.ElevateDashboard || {});
  NS.version = "phase11-loader-consolidation-v1";
  ...
  const MODULES = [
    ...
    "/dashboard-phase11-consolidated.js?v=20260411p11"
  ];
  ...
  function installLateDOMContentLoadedCompat() {
    ...
    document.addEventListener = function (type, listener, options) {
      if (type === "DOMContentLoaded" && typeof listener === "function" && document.readyState !== "loading") {
        try { queueMicrotask(() => { try { listener.call(document, new Event("DOMContentLoaded")); } catch (error) { console.error("[Elevate Dashboard] Late DOMContentLoaded listener failed:", error); } }); }
        catch { setTimeout(() => { try { listener.call(document, new Event("DOMContentLoaded")); } catch (innerError) { console.error("[Elevate Dashboard] Late DOMContentLoaded listener failed:", innerError); } }, 0); }
  ...
  loadScriptSequentially()
    .then(() => { installControlledBootKick(); setLoaderState("modules-loaded"); })
    .catch((error) => {
