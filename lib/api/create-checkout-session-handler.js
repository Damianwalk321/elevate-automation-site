...
import { requireVerifiedDashboardUser, getTrustedIdentity } from "../../api/_shared/auth.js";
import { getPlanConfig } from "../../_shared/offer-config.js";
...
function getSiteUrl(req) {
  return process.env.SITE_URL || process.env.NEXT_PUBLIC_SITE_URL || `${req.headers["x-forwarded-proto"] || "https"}://${req.headers.host}`;
}
...
function getTrialEndUnix(trialDays) {
...
async function findOrCreateCustomer({ stripeClient, email, userId }) {
  ...
  const existing = await stripeClient.customers.list({ email: normalizedEmail, limit: 1 });
  ...
  if (existing?.data?.length) {
    ...
    if (JSON.stringify(nextMetadata) !== JSON.stringify(customer.metadata || {})) {
      return await stripeClient.customers.update(customer.id, { metadata: nextMetadata });
    }
    return customer;
  }
  ...
  return await stripeClient.customers.create({
    ...
    metadata: { email: normalizedEmail, user_id: clean(userId || "") }
  });
}
...
async function mirrorCustomerToSupabase({ email, userId, stripeCustomerId }) {
  ...
  const payload = { stripe_customer_id: stripeCustomerId, updated_at: new Date().toISOString() };
...
async function getLockedReferralFromSupabase({ email, userId }) {
  const supabaseUrl = process.env.SUPABASE_URL;
  const supabaseServiceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!supabaseUrl || !supabaseServiceRoleKey) return { referralCode: "", referralSource: "" };
...
export default async function checkoutSessionHandler(req, res) {
  ...
  try {
    ...
    if (body.__parse_error) return json(res, 400, { error: body.__parse_error });
    ...
    if (!plan) return json(res, 400, { error: "Invalid or unsupported plan type", debug: { planType, userType, accessType } });
    if (!plan.priceId) return json(res, 500, { error: `Missing ${plan.lookupKey}` });

    const customer = await findOrCreateCustomer({ stripeClient: stripe, email, userId });
    await mirrorCustomerToSupabase({ email, userId, stripeCustomerId: customer.id });

    const trialEnd = getTrialEndUnix(plan.trialDays);
    ...
    return json(res, 500, { error: error?.message || "Failed to create checkout session" });
  }
}
