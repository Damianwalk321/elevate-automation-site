...
function buildListingIntelligence(row = {}) {
  ...
  const staleLike =
    status === "stale" ||
    lifecycle === "stale" ||
    lifecycle === "review_delete" ||
    reviewBucket === "removedvehicles";
  ...
  let opportunityScore = Math.round((predictedScore * 0.45) + Math.min(views, 30) + Math.min(messages * 15, 30));
  ...
  return {
    ...
    predicted_label:
      predictedScore >= 75 ? "High Performer" :
      predictedScore >= 55 ? "Likely Performer" :
      predictedScore < 35 ? "Low Probability" : "Uncertain",
    pricing_insight:
      highViewsNoMessages ? "Price may be limiting message conversion." :
      messages >= 2 ? "Pricing appears competitive." :
      "Pricing signal still developing.",
    content_score: contentScore,
    content_feedback:
      contentScore >= 80 ? "Listing structure looks strong." :
      contentScore >= 65 ? "Content is workable but could be tightened." :
    ...
    action_bucket_label:
      actionBucket === "do_now" ? "Do Now" :
      actionBucket === "do_today" ? "Do Today" :
      actionBucket === "watch" ? "Watch" : "Low Priority"
  };
}
...
function preferListingRow(current, incoming) {
  ...
  const currentScore =
    ...
    safeNumber(current.views_count) +
    safeNumber(current.messages_count) * 10 +
    new Date(current.updated_at || current.posted_at || 0).getTime() / 1000000000000;

  const incomingScore =
    (incoming.source_table === "user_listings" ? 1000 : 0) +
    safeNumber(incoming.views_count) +
    safeNumber(incoming.messages_count) * 10 +
    new Date(incoming.updated_at || incoming.posted_at || 0).getTime() / 1000000000000;
...
async function fetchTableRows(tableName, userIds = [], emails = [], warnings = []) {
  ...
  async function pushRows(query) {
    ...
    for (const row of (Array.isArray(data) ? data : [])) {
      const key =
        clean(row?.id || "") ||
        `${clean(row?.marketplace_listing_id || "")}|${clean(row?.posted_at || row?.created_at || "")}`;
...
function sortRows(rows, sort) {
  ...
  if (sort === "predicted") return items.sort((a, b) => safeNumber(b.predicted_score) - safeNumber(a.predicted_score));
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const finalUserId = clean(identity.primary_user_id || userId || "");
    const finalEmail = normalizeEmail(identity.primary_email || email || "");

    if (!finalUserId && !finalEmail) {
      return res.status(400).json({ error: "Missing userId or email", request_id: requestId });
    }
    ...
    let rows = [...mergedMap.values()].filter((row) =>
      matchesFilter(row, { status, lifecycleStatus, reviewBucket, search, preset })
    );
    ...
    return res.status(200).json({
      ...
      meta: {
        ...
        backfill: {
          enabled: shouldBackfill,
          user_listings_updated: userBackfill.updated,
          listings_updated: legacyBackfill.updated
        }
      }
    });
  } catch (error) {
