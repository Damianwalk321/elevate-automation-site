...
function buildListingIntelligence(row={}){
  ...
  const needsAction=weak||highViewsNoMessages||lifecycle==='review_price_update'||bucket==='pricechanges';
  let healthScore=100-Math.min(ageDays*2,30)+Math.min(messages*16,40)+Math.min(views,20);
  ...
  else if(lifecycle==='review_price_update'||bucket==='pricechanges'||highViewsNoMessages) recommendedAction='Review price';
  else if(lifecycle==='review_new'||bucket==='newvehicles') recommendedAction='Review new listing';
  ...
  let postPriority = 35;
  if (!postedAt) postPriority += 28;
  if (lifecycle === 'review_new' || bucket === 'newvehicles') postPriority += 24;
  ...
  if (highViewsNoMessages || lifecycle === 'review_price_update' || bucket === 'pricechanges') priceReviewPriority += 52;
  ...
  const actionLabel = actionBucket === 'do_now' ? 'Do Now' : actionBucket === 'do_today' ? 'Do Today' : actionBucket === 'watch' ? 'Watch' : 'Low Priority';
  return { age_days:ageDays, likely_sold:likelySold, promote_now:promoteNow, weak, needs_action:needsAction, health_score:healthScore, health_label:healthLabel, recommended_action:recommendedAction, predicted_score:predictedScore, predicted_label:predictedLabel, pricing_insight:pricingInsight, content_score:contentScore, content_feedback:contentFeedback, popularity_score:(messages*1000)+(views*10)+(postedAt?new Date(postedAt).getTime()/100000000:0), post_priority: postPriority, refresh_priority: refreshPriority, price_review_priority: priceReviewPriority, opportunity_score: opportunityScore, action_bucket: actionBucket, action_bucket_label: actionLabel };
}
