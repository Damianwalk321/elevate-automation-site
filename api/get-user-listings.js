...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!verifiedUser?.id || !verifiedUser?.email) return res.status(401).json({ error: "Unauthorized", request_id: requestId });
    ...
    const identity = await resolveIdentityCandidates({ userId, email, authUid: clean(verifiedUser.id || ""), warnings });
    const finalUserId = clean(identity.primary_user_id || userId || "");
    const finalEmail = normalizeEmail(identity.primary_email || email || "");
    if (!finalUserId && !finalEmail) return res.status(400).json({ error: "Missing userId or email", request_id: requestId });
    ...
    const [userBackfill, legacyBackfill] = await Promise.all([
    ...
    return res.status(200).json({
      ...
      meta: {
        ...
        backfill: { user_listings_updated: userBackfill.updated, listings_updated: legacyBackfill.updated }
      }
    });
  } catch (error) {
    console.error("get-user-listings fatal error:", error);
    return res.status(500).json({ error: error.message || "Internal server error", request_id: requestId, warnings });
  }
}
