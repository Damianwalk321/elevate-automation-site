(() => {
  ...
  function readSummaryFallback() {
    ...
    for (let i = 0; i < Math.min(activeListings, 5); i += 1) {
      items.push({ id: `summary_listing_${i + 1}`, title: `Tracked Listing ${i + 1}`, price: "", views: avgViews, messages: avgMessages, source: "summary_fallback", status: "active", last_seen_at: new Date().toISOString(), sync_source: "summary_fallback", sync_confidence: "estimated" });
    }
    return items;
  }
  ...
  function buildAnalyticsFromRegistry() {
    ...
    const buckets = {
      ...
      price_attention: listings.filter((item) => item.health_state === "price_attention" || item.price_review_required || clean(item.lifecycle_status).toLowerCase() === "review_price_update").slice(0, 5),
      ...
      recovered: listings.filter((item) => { const listingEvents = state?.getListingEvents?.(item.id) || []; return listingEvents.some((evt) => evt.type === "price_changed") && listingEvents.some((evt) => evt.type === "message_update"); }).slice(0, 5)
    };
  ...
  function rebuildRegistryFromFallback() {
    ...
    const currentIds = [];
    items.forEach((item, idx) => { const next = { ...item }; if (!next.id) next.id = `${next.title || "listing"}_${idx + 1}`; const upserted = state.upsertListing(next, { silent: true }); currentIds.push(upserted.id); });
  ...
  function applyHealthToRegistry() {
    ...
    registry.forEach((item) => { const listingEvents = state?.getListingEvents?.(item.id) || []; item.health_state = inferHealth(item, listingEvents); item.confidence = confidenceFor(item); state.upsertListing(item, { silent: true, skipEvents: true, skipPersist: true }); });
  }
  ...
  function isReviewItem(item) {
    ...
    return lifecycle.startsWith("review_") || bucket === "removedvehicles" || bucket === "pricechanges" || !!item.needs_action || !!item.weak || !!item.price_review_required || !!item.missing_image;
  }
  ...
  function cardBadge(item) {
    ...
    if (item.price_review_required || lifecycle === 'review_price_update') return '<span class="badge warn">Price Review</span>';
    if (item.missing_image) return '<span class="badge warn">Image Missing</span>';
  ...
  function listingCard(item) {
    ...
    return `
      <article class="listing-card ${warning ? 'truth-warning' : ''}" data-hydrated="true">
        <div class="listing-media">
          ${item.image_url ? `<img src="${item.image_url}" alt="${clean(item.title)}" />` : '<div class="listing-placeholder">Image not synced</div>'}
        ...
        <div class="listing-content">
          ...
          <div class="listing-truth-row">
            <span class="listing-truth-chip">${formatMileage(item)}</span>
            <span class="listing-truth-chip">Posted ${formatDate(item.posted_at)}</span>
            <span class="listing-truth-chip">Seen ${formatDate(item.last_seen_at)}</span>
            ${item.price_warning ? `<span class="listing-truth-chip warn">${clean(item.price_warning).split('|')[0].replace(/_/g, ' ')}</span>` : ''}
          </div>
  ...
  function reviewCard(item) {
    return `<article class="overview-action-item" data-hydrated-review="true"><div><div class="title">${clean(item.title || 'Listing')}</div><div class="sub">${clean(item.recommended_action || 'Review listing')} • ${clean(item.stock_number || item.vin || '')}</div></div><div>${cardBadge(item)}</div></article>`;
  }
  ...
  function bindControls() {
    ...
    if (filterWrap && filterWrap.dataset.hydratedBindings !== 'true') {
      ...
      buttons.forEach((btn) => btn.addEventListener('click', () => { listingUi.filter = btn.getAttribute('data-filter') || 'all'; buttons.forEach((b) => b.classList.toggle('active', b === btn)); renderListingsSection(); renderReviewCenter(); }));
    }
    if (sortSelect && sortSelect.dataset.hydratedBindings !== 'true') {
      sortSelect.dataset.hydratedBindings = 'true';
      sortSelect.addEventListener('change', () => { const value = clean(sortSelect.value || '').toLowerCase(); if (value.includes('new')) listingUi.sort = 'newest'; else if (value.includes('price_high')) listingUi.sort = 'price_high'; else if (value.includes('price_low')) listingUi.sort = 'price_low'; else listingUi.sort = 'popular'; renderListingsSection(); });
    }
    if (search && search.dataset.hydratedBindings !== 'true') {
      search.dataset.hydratedBindings = 'true';
      search.addEventListener('input', () => { listingUi.search = search.value || ''; renderListingsSection(); });
    }
  }
  ...
  async function hydrateListings() {
    ...
    if (apiPayload?.listings?.length) { ingestRemotePayload(apiPayload); return true; }
    const remote = getWindowRemotePayload();
    if (remote) { ingestRemotePayload(remote); return true; }
  ...
  function bindRefresh() {
    ...
    btn.addEventListener('click', async () => { await hydrateListings(); window.dispatchEvent(new CustomEvent('elevate:tracking-refreshed')); });
  }
